...
/**
 * Компонент для загрузки защищенных изображений (Thumbnail)
 * Использует api с JWT для получения Blob
 */
const SecureThumbnail = ({ url, title }) => {
  ...
  if (!imageSrc) {
    return <div style={placeholderStyle} />; // Состояние загрузки (пустой блок)
  }

  return <img src={imageSrc} alt={title} style={imageStyle} />;
};
...
const nicknameStyle = {
  fontSize: '0.85rem', color: '#cbd5e1', textAlign: 'center', width: '100%', 
...
const videoCardStyle = {
  ...
  cursor: 'pointer', border: '1px solid #334155', transition: 'transform 0.2s'
};
...
const arrowButtonStyle = {
  ...
  transition: '0.3s ease', boxShadow: '0 4px 12px rgba(0,0,0,0.5)'
};
